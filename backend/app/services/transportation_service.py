...
class TransportationService:
    ...
    async def get_transportation_by_id(self, transportation_id: uuid.UUID | str) -> Optional[Transportation]:
        return await self.repository.get_by_id(transportation_id)

    async def get_transportations(self, skip: int = 0, limit: int = 100) -> List[Transportation]:
        return await self.repository.get_all(skip=skip, limit=limit)

    async def create_transportation(self, transportation_in: TransportationCreate, **kwargs: Any) -> Transportation:
    ...
    async def update_transportation(self, db_obj: Transportation, transportation_in: TransportationUpdate) -> Transportation:
