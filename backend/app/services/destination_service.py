...
class DestinationService:
    ...
    async def get_destination_by_id(self, destination_id: uuid.UUID | str) -> Optional[Destination]:
        return await self.repository.get_by_id(destination_id)

    async def get_destinations(self, skip: int = 0, limit: int = 100) -> List[Destination]:
        return await self.repository.get_all(skip=skip, limit=limit)

    async def create_destination(self, destination_in: DestinationCreate, **kwargs: Any) -> Destination:
    ...
    async def update_destination(self, db_obj: Destination, destination_in: DestinationUpdate) -> Destination:
