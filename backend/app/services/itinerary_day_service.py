...
class ItineraryDayService:
    ...
    async def get_itinerary_day_by_id(self, itinerary_day_id: uuid.UUID | str) -> Optional[ItineraryDay]:
        return await self.repository.get_by_id(itinerary_day_id)

    async def get_itinerary_days(self, skip: int = 0, limit: int = 100) -> List[ItineraryDay]:
        return await self.repository.get_all(skip=skip, limit=limit)

    async def create_itinerary_day(self, itinerary_day_in: ItineraryDayCreate, **kwargs: Any) -> ItineraryDay:
    ...
    async def update_itinerary_day(self, db_obj: ItineraryDay, itinerary_day_in: ItineraryDayUpdate) -> ItineraryDay:
